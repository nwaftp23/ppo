...
class Scaler(object):
    ...
    def update(self, x, r):
        ...
        if self.first_pass:
            ...
            new_means_rew = ((self.mean_rew * self.n_rew) + (new_data_mean * m_rew)) / (self.n_rew + m_rew)
            self.var_rew = (((self.n_rew * (self.var_rew + np.square(self.mean_rew))) +
                          (m_rew * (new_data_var_rew + np.square(new_data_mean_rew)))) / (self.n_rew + m_rew) -
                         np.square(new_means_rew))
            print('Is variance less than 0', self.vars_rew < 0)
