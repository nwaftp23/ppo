...
def main(env_name, num_episodes, gamma, lam, kl_targ, batch_size, hid1_mult, policy_logvar):
    ...
    while episode < num_episodes:
        ...
        disc0 = [trajectories['rewards'][0] for t in trajectories]
        print(disc0)
        inpuit('check')
        ...
        rew_graph = np.append(rew_graph,disc_sum_rew)
        print('rew_graph', rew_graph)
        ...
        plt.close()
        mean_rew_graph = np.append(mean_rew_graph,np.mean(disc_sum_rew))
        print('mean_rew_graph', mean_rew_graph)
        ...
        plt.close()
    tr = run_policy(env, policy, scaler, logger, episodes=10000)
    sum_rewww = [t['rewards'].sum() for t in tr]
    hist_dat = np.array(sum_rewww)
    print(hist_dat.shape)
    fig = plt.hist(hist_dat,bins=100, edgecolor='b', linewidth=1.2)
    plt.title('Standard PPO')
